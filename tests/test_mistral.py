...
# Mistral names OCR releases by date rather than by generation, so these ids are
# easy to get wrong — and a wrong one is a 404 at the first call. Set `model` to
# whatever your account serves; on Azure Foundry it is mistral-document-ai-*.
KNOWN_OCR_MODELS = {
    "mistral-ocr-latest",
...
def test_the_default_models_are_dated_ids_not_generation_numbers(parser_id):
    model = registry.get(parser_id).model_default
    assert model in KNOWN_OCR_MODELS, f"{model} is not an id Mistral publishes"
